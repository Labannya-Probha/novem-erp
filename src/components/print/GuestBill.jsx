  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', position: 'relative', padding: '10px' }}>
      ...
      <table style={{ width: '100%' }}>
        <tbody>
          <tr>
            <td style={{ width: '30%', verticalAlign: 'top' }}></td>
      ...
      <table style={{ width: '100%', borderBottom: '1.5px solid #2E7D32', paddingBottom: 6, fontSize: 11, fontFamily: '"IBM Plex Mono", monospace' }}>
        <tbody>
          <tr>
            <td style={{ textAlign: 'left' }}><b>Invoice No:</b> {invNo}</td>
            <td style={{ textAlign: 'right' }}><b>Date:</b> {fmtDate(issuedDate)}</td>
          </tr>
        </tbody>
      </table>

      <div style={{ textAlign: 'center', marginTop: 24, marginBottom: 8 }}>
        <span style={{ fontSize: 15, fontWeight: 700, letterSpacing: 2, color: '#2E7D32', borderBottom: '2px solid #2E7D32', paddingBottom: 4, textTransform: 'uppercase' }}>GUEST BILL</span>
      </div>

      <table style={{ width: '100%', borderTop: '1px solid #e0e0e0', borderBottom: '1px solid #e0e0e0', padding: '12px 0', fontSize: 11, marginTop: 12 }}>
        <tbody>
          <tr>
            <td style={{ padding: '4px 0', verticalAlign: 'top' }}><b>Guest:</b> {bName}<br /><b>Address:</b> {bAddress}</td>
            <td style={{ padding: '4px 0', textAlign: 'right', verticalAlign: 'top' }}><b>Reservation:</b> {res?.res_no}<br /><b>Stay:</b> {fmtDate(res?.check_in)} → {fmtDate(res?.check_out)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', marginTop: 20, borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            ...
            <th style={hcell}>Base</th>
            <th style={hcell}>Discount</th>
            <th style={hcell}>Service Chg.</th>
            <th style={hcell}>SD</th>
            <th style={hcell}>VAT</th>
...
const TR = ({ k, v }) => (
  <tr>
    <td style={{ padding: '5px 6px', fontSize: 11 }}>{k}</td>
    <td style={{ padding: '5px 6px', textAlign: 'right', fontFamily: '"IBM Plex Mono", monospace', fontSize: 11 }}>{v}</td>
  </tr>
)
